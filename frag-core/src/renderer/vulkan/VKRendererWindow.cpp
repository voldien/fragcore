...
//	vkCreateWin32SurfaceKHR
	...
	result = vkGetSwapchainImagesKHR(vulkancore->device, swapChain.swapchain, &nrChainImageCount,
	...
	for (size_t i = 0; i < swapChain.swapChainImages.size(); i++) {
		...
		if (vkCreateImageView(vulkancore->device, &createInfo, nullptr, &swapChain.swapChainImageViews[i]) !=
			VK_SUCCESS) {
	...
	for (size_t i = 0; i < swapChain.swapChainImageViews.size(); i++) {
		...
		if (vkCreateFramebuffer(vulkancore->device, &framebufferInfo, nullptr, &swapChain.swapChainFramebuffers[i]) !=
			VK_SUCCESS) {
	...
	result = vkAllocateCommandBuffers(vulkancore->device, &cmdBufAllocInfo, swapChain.commandBuffers.data());
	if (result != VK_SUCCESS)
	...
	if (vkAllocateCommandBuffers(vulkancore->device, &allocInfo, swapChain.commandBuffers.data()) != VK_SUCCESS) {
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
		if (vkCreateSemaphore(vulkancore->device, &semaphoreInfo, nullptr, &vulkancore->imageAvailableSemaphores[i]) !=
				VK_SUCCESS ||
			vkCreateSemaphore(vulkancore->device, &semaphoreInfo, nullptr, &vulkancore->renderFinishedSemaphores[i]) !=
				VK_SUCCESS ||
