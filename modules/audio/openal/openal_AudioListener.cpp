...
float OpenALAudioListener::getVolume() const {
	float volume;
	FAOPAL_VALIDATE(alGetListenerf(AL_GAIN, &volume));
	return volume;
}

void OpenALAudioListener::pause(bool pause) { this->setVolume(0.0f); }

void OpenALAudioListener::setPosition(const Vector3 &position) {
	const Vector3 *p = &position;
	Vector3 prevPos = getPosition();
	FAOPAL_VALIDATE(alListenerfv(AL_POSITION, (const ALfloat *)&p[0]));
	setVelocity(position - prevPos);
}

const Vector3 OpenALAudioListener::getPosition() const {
	Vector3 position(0.0f, 0.0f, 0.0f);
	FAOPAL_VALIDATE(alGetListenerfv(AL_POSITION, &position[0]));
	return position;
}

void OpenALAudioListener::setVelocity(const Vector3 &velocity) {
	const Vector3 *v = &velocity;
	FAOPAL_VALIDATE(alListenerfv(AL_VELOCITY, (const ALfloat *)&v[0]));
}

const Vector3 OpenALAudioListener::getVelocity() const {
	Vector3 velocity;
	FAOPAL_VALIDATE(alGetListenerfv(AL_VELOCITY, &velocity[0]));
	return velocity;
}
