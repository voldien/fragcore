...
// 	/*  Create swapchain.   */
// 	if (vkCreateSwapchainKHR(vulkancore->device->getHandle(), &createInfo, nullptr, &swapChain.swapchain) != VK_SUCCESS) {
...
// 	result = vkGetSwapchainImagesKHR(vulkancore->device->getHandle(), swapChain.swapchain, &nrChainImageCount, nullptr);
...
// 	if (result != VK_SUCCESS)
// 		throw RuntimeException(fmt::format("vkGetSwapchainImagesKHR failed query object - %d", result));
...
// 	if (vkCreateRenderPass(vulkancore->device->getHandle(), &renderPassInfo, nullptr, &swapChain.renderPass) != VK_SUCCESS) {
...
// 		if (vkCreateFramebuffer(vulkancore->device->getHandle(), &framebufferInfo, nullptr, &swapChain.swapChainFramebuffers[i]) !=
// 			VK_SUCCESS) {
...
// 	result = vkAllocateCommandBuffers(vulkancore->device->getHandle(), &cmdBufAllocInfo, swapChain.commandBuffers.data());
// 	if (result != VK_SUCCESS)
// 		throw RuntimeException(fmt::format("vkAllocateCommandBuffers failed - %d", result));
...
// 	// 		fmt::format("vkBeginCommandBuffer failed - %d", result));
// 	swapChain.commandBuffers.resize(swapChain.swapChainFramebuffers.size());
...
// 	if (vkAllocateCommandBuffers(vulkancore->device->getHandle(), &allocInfo, swapChain.commandBuffers.data()) != VK_SUCCESS) {
...
// 	vulkancore->imagesInFlight.resize(swapChain.swapChainImages.size(),
// 									  VK_NULL_HANDLE); // TODO resolve vulkancore->swapChainImages.size()
...
// 		if (vkCreateSemaphore(vulkancore->device->getHandle(), &semaphoreInfo, nullptr, &vulkancore->imageAvailableSemaphores[i]) !=
// 				VK_SUCCESS ||
// 			vkCreateSemaphore(vulkancore->device->getHandle(), &semaphoreInfo, nullptr, &vulkancore->renderFinishedSemaphores[i]) !=
// 				VK_SUCCESS ||
// 			vkCreateFence(vulkancore->device->getHandle()->getHandle, &fenceInfo, nullptr, &vulkancore->inFlightFences[i]) != VK_SUCCESS) {
...
// 		vkCmdClearColorImage(swapChain.commandBuffers[i], swapChain.swapChainImages[i], VK_IMAGE_LAYOUT_GENERAL,
...
void VKRenderWindow::swapBuffer() {
	...
	result = vkAcquireNextImageKHR(getDevice(), swapChain.swapchain, UINT64_MAX,
								   this->imageAvailableSemaphores[swapChain.currentFrame], VK_NULL_HANDLE, &imageIndex);
