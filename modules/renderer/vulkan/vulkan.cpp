...
// 		throw RuntimeException(fmt::format("Unsuccessful VkResult: %s", getVKResultString(result)));
// 	}
// }
...
// 		if ((typeFilter & (1 << i)) &&
// 			(vulkanCore->memProperties.memoryTypes[i].propertyFlags & properties) == properties) {
// 			return i;
// 		}
// 	}
// }

// void createBuffer(VulkanCore *vulkanCore, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags
// properties, 				  VkBuffer &buffer, VkDeviceMemory &bufferMemory) { 	VkResult result;
...
// void selectDefaultDevices(std::vector<VkPhysicalDevice> &devices, std::vector<VkPhysicalDevice> &selectDevices) {
// 	/*  Check for matching. */
...
unsigned int getTextureFormat(TextureDesc::Format format) {
	...
	default:
		throw InvalidArgumentException("Invalid texture fmt::format.");
	}
}
